...
class TelefoneAlunoDAO(_ITelefoneDAO.ITelefoneDAO):
    ...
    def atualizar(self, db: _orm.Session, telefone: _telefoneModel.TelefoneAluno) -> _telefoneModel.TelefoneAluno:
        db.query(_telefoneModel.TelefoneAluno).filter(_telefoneModel.TelefoneAluno.id == telefone.id).update(telefone)
        db.commit()
        db.refresh(telefone)
        return telefone
    
    def remover(self, db: _orm.Session, telefone_id: int) -> _telefoneModel.TelefoneAluno:
        telefone = db.query(_telefoneModel.TelefoneAluno).filter(_telefoneModel.TelefoneAluno.id == telefone_id).first()
        db.remove(telefone)
        db.commit()
        return telefone
    
    def buscar(self, db: _orm.Session, telefone_id: int) -> _telefoneModel.TelefoneAluno:
        telefone = db.query(_telefoneModel.TelefoneAluno).filter(_telefoneModel.TelefoneAluno.id == telefone_id).first()
        return telefone
...
class TelefoneProfessorDAO(_ITelefoneDAO.ITelefoneDAO):
    ...
    def atualizar(self, db: _orm.Session, telefone: _telefoneModel.TelefoneProfessor) -> _telefoneModel.TelefoneProfessor:
        db.query(_telefoneModel.TelefoneProfessor).filter(_telefoneModel.TelefoneProfessor.id == telefone.id).update(telefone)
        db.commit()
        db.refresh(telefone)
        return telefone
    
    def remover(self, db: _orm.Session, telefone_id: int) -> _telefoneModel.TelefoneProfessor:
        telefone = db.query(_telefoneModel.TelefoneProfessor).filter(_telefoneModel.TelefoneProfessor.id == telefone_id).first()
        db.remove(telefone)
        db.commit()
        return telefone
    
    def buscar(self, db: _orm.Session, telefone_id: int) -> _telefoneModel.TelefoneProfessor:
        telefone = db.query(_telefoneModel.TelefoneProfessor).filter(_telefoneModel.TelefoneProfessor.id == telefone_id).first()
        return telefone
    ...
    def buscarPorDdd(self, db: _orm.Session, ddd: int) -> _telefoneModel.TelefoneProfessor:
        telefone = db.query(_telefoneModel.TelefoneProfessor).filter(_telefoneModel.TelefoneProfessor.ddd == ddd).first()
        return telefone
