...
class DisciplinaDAO(_IDisciplinaDAO.IDisciplinaDAO):
    def inserir(self, db: _orm.Session, disciplina: _disciplinaModel.Disciplina) -> _disciplinaSchema.Disciplina:
        db.add(disciplina)
        db.commit()
    ...
    def buscar(self, db: _orm.Session, id_disciplina: int) -> _disciplinaModel.Disciplina:
        disciplina = (
            db.query(_disciplinaModel.Disciplina)
            .filter(_disciplinaModel.Disciplina.id_disciplina == id_disciplina)
            .first()
