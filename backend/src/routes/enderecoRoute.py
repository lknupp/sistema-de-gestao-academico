...
class Route(_IRoute.IRoute):
    @router.post(
        "/api/endereco/",
        response_model=_enderecoSchema.Endereco,
    ...
    @router.get(
        "/api/endereco/",
        response_model=List[_enderecoSchema.Endereco],
        tags=["endereco"],
    )
    ...
    @router.put(
        "/api/endereco/", response_model=_enderecoSchema.Endereco, tags=["endereco"]
    )
    def atualizar(
    ...
    def remover(
        endereco_id: int, db: _orm.Session = _fastapi.Depends(_database.get_db)
    ):
