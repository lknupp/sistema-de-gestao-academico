...
class AlunoRoute(_IRoute.IRoute):
    @router.post("/api/aluno/", response_model=_alunoSchema.Aluno, status_code=HTTPStatus.CREATED.value, description=HTTPStatus.CREATED.phrase, tags=['aluno'])
    ...
    @router.get("/api/aluno/id/{id_aluno}", response_model=_alunoSchema.Aluno, tags=['aluno'])
    def ler(id_aluno: int, db: _orm.Session = _fastapi.Depends(_database.get_db)):
        return controller.buscar(db, id_aluno)

    @router.get("/api/aluno/cpf/{cpf}", response_model=_alunoSchema.Aluno, tags=['aluno'])
