...
class ProfessorRoute(_IRoute.IRoute):
    @router.post(
        "/api/professor/",
        response_model=_professorSchema.Professor,
    ...
    @router.get(
        "/api/professor/",
        response_model=List[_professorSchema.Professor],
        tags=["professor"],
    )
    ...
    @router.put(
        "/api/professor/", response_model=_professorSchema.Professor, tags=["professor"]
    )
    def atualizar(
        professor: _professorSchema.Professor,
        db: _orm.Session = _fastapi.Depends(_database.get_db),
    ):
    ...
    def remover(
        id_professor: int, db: _orm.Session = _fastapi.Depends(_database.get_db)
    ):
